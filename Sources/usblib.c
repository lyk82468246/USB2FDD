...
static void FDD_USB_Abort(void);
static void FDD_USB_Quiesce(void);
static void FDD_USB_SessionBegin(void);
static void FDD_USB_SessionEnd(void);
static void FDD_USB_SendSessionStatus(void);
static void FDD_USB_WriteForceOff(void);
...
static void FDD_USB_WriteGateOffIndex(void);
...
static uint8_t FDD_USB_ParseThreeU8(const char *text, uint8_t *first, uint8_t *second, uint8_t *third)
{
    ...
    if (!FDD_USB_ParseU8(text, second))
        return 0;
...
static void FDD_USB_SessionBegin(void)
{
    ...
    FDD_IO_Motor(1);
    FDD_Index_Reset();
    FDD_USB_SendText("OK SESSION_BEGIN\r\n");
}
...
static void FDD_USB_SessionEnd(void)
{
    ...
    FDD_IO_Select(0);
    FDD_IO_Motor(0);
    FDD_USB_SendText("OK SESSION_END\r\n");
}

static void FDD_USB_SendSessionStatus(void)
{
    sprintf(g_usb_text_resp,
            "SESSION SEL=%u MOT=%u CAP=%u",
            FDD_IO_IsSelected(),
            FDD_IO_IsMotorOn(),
            FDD_Flux_IsCapturing());
    FDD_USB_SendText(g_usb_text_resp);
...
static void FDD_USB_WriteArm(void)
{
    ...
    {
        FDD_USB_SendText("ERR WRITE_ARM NO_SELECT\r\n");
        return;
    }

    if (!FDD_IO_IsMotorOn())
    {
        FDD_USB_SendText("ERR WRITE_ARM NO_MOTOR\r\n");
        return;
    }

    if (FDD_IO_IsWriteProtected())
    {
        FDD_USB_SendText("ERR WRITE_ARM WRITE_PROTECT\r\n");
        return;
    }

    g_usb_write_armed = 1;
    FDD_USB_SendText("OK WRITE_ARM\r\n");
}

static void FDD_USB_WriteDisarm(void)
{
    FDD_USB_WriteForceOff();
    FDD_USB_SendText("OK WRITE_DISARM\r\n");
}
...
static void FDD_USB_WriteGateOn(void)
{
    ...
    {
        FDD_USB_SendText("ERR WRITE_GATE_ON NOT_ARMED\r\n");
        return;
    }
    ...
    {
        FDD_USB_WriteForceOff();
        FDD_USB_SendText("ERR WRITE_GATE_ON NO_SELECT\r\n");
        return;
    }
    ...
    {
        FDD_USB_WriteForceOff();
        FDD_USB_SendText("ERR WRITE_GATE_ON NO_MOTOR\r\n");
        return;
    }
    ...
    {
        FDD_USB_WriteForceOff();
        FDD_USB_SendText("ERR WRITE_GATE_ON WRITE_PROTECT\r\n");
        return;
    }
...
static void FDD_USB_WriteGateOffIndex(void)
{
    ...
    {
        FDD_USB_SendText("ERR WRITE_GATE_OFF_INDEX NO_WRITE_GATE\r\n");
        return;
    }
    ...
    {
        FDD_USB_WriteForceOff();
        FDD_USB_SendText("ERR WRITE_GATE_OFF_INDEX NO_INDEX\r\n");
        return;
    }
    ...
    g_usb_write_gate_on = 0;
    g_usb_write_data_active = 0;
    FDD_USB_SendText("OK WRITE_GATE_OFF_INDEX\r\n");
}
...
static void FDD_USB_WritePulses(uint8_t count, uint8_t pulse_us, uint8_t gap_us)
{
    ...
    if (!g_usb_write_gate_on)
    {
        FDD_USB_SendText("ERR WRITE_PULSES NO_WRITE_GATE\r\n");
        return;
    }

    if ((count == 0) || (count > FDD_WRITE_PULSES_MAX) ||
        (pulse_us == 0) || (pulse_us > FDD_WRITE_PULSE_MAX_US))
    {
        FDD_USB_SendText("ERR WRITE_PULSES RANGE\r\n");
        return;
    }
...
static void FDD_USB_SendHelp(void)
{
    ...
    FDD_USB_SendText("CMDS SAFE ABORT QUIESCE SESSION_BEGIN SESSION_END SESSION_STATUS READY MOTOR_ON MOTOR_OFF\r\n");
...
void FDD_USB_ProcessPacket(uint8_t *buf, uint16_t len)
{
    ...
    else if (FDD_USB_CmdEq(g_usb_cmd, "ABORT"))
    {
        FDD_USB_Abort();
    }
    else if (FDD_USB_CmdEq(g_usb_cmd, "QUIESCE"))
    {
        FDD_USB_Quiesce();
    }
    else if (FDD_USB_CmdEq(g_usb_cmd, "SESSION_BEGIN"))
    {
        FDD_USB_SessionBegin();
    }
    else if (FDD_USB_CmdEq(g_usb_cmd, "SESSION_END"))
    {
        FDD_USB_SessionEnd();
    }
    else if (FDD_USB_CmdEq(g_usb_cmd, "SESSION_STATUS"))
    {
        FDD_USB_SendSessionStatus();
    }
    ...
    else if (FDD_USB_CmdEq(g_usb_cmd, "WRITE_ARM"))
    {
        FDD_USB_WriteArm();
    }
    else if (FDD_USB_CmdEq(g_usb_cmd, "WRITE_DISARM"))
    {
        FDD_USB_WriteDisarm();
    }
    ...
    else if (FDD_USB_CmdEq(g_usb_cmd, "WRITE_GATE_OFF_INDEX"))
    {
        FDD_USB_WriteGateOffIndex();
    }
    else if (FDD_USB_CmdStarts(g_usb_cmd, "WRITE_DATA "))
    {
        FDD_USB_WriteData(g_usb_cmd[11] == '1');
    }
    else if (FDD_USB_CmdStarts(g_usb_cmd, "WRITE_PULSES "))
    {
        if (FDD_USB_ParseThreeU8(&g_usb_cmd[13], &value, &value2, &i))
            FDD_USB_WritePulses(value, value2, i);
        else
            FDD_USB_SendText("ERR WRITE_PULSES ARG\r\n");
    }
    ...
    else if (FDD_USB_CmdEq(g_usb_cmd, "DISK_STATUS"))
    {
        FDD_USB_SendDiskStatus();
    }
    else if (FDD_USB_CmdEq(g_usb_cmd, "WRITE_STATUS"))
    {
        FDD_USB_SendWriteStatus();
    }
